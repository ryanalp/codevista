...
function classifyVariables(variables: TraceVariable[]) {
    const primitives: TraceVariable[] = [];
    const dataStructures: TraceVariable[] = [];

    for (const v of variables) {
        if (Array.isArray(v.value) || isNodeRepr(v.value)) {
            dataStructures.push(v);
        } else {
            primitives.push(v);
        }
    }

    return { primitives, dataStructures };
}
...
export default function VisualStage({
    ...
    const { primitives, dataStructures } = classifyVariables(currentFrame.variables);
        ...
        <div className="flex flex-col h-full bg-zinc-950 border border-zinc-800 rounded-xl p-3 sm:p-6 overflow-y-auto">
            ...
            {currentFrame.variables.length === 0 ? (
                ...
                <div className="flex flex-col gap-6 flex-1">
                    ...
                    {primitives.length > 0 && (
                        <section>
                            <p className="text-[10px] font-bold tracking-widest text-zinc-600 uppercase mb-3">
                                Primitives
                            </p>
                            <AnimatePresence mode="popLayout">
